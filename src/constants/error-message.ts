...
export const ErrorMessage = {
  Generic: {
    ...
    IncorrectResponse:
      "Your request succeeded, but we couldn't finalize the response",
    RequestTimeout: (ms: number) => `Request timed out after ${ms}ms`,
  },
  Fields: {
    KeyMustBeValidT: (key: string, type: string) =>
      `${key} must be a valid ${type}`,
    ...
    KeyMustHaveMinValue: (key: string, value: number) =>
      `${key} must be at least ${value}`,
    KeyMaxHaveMaxValue: (key: string, value: number) =>
      `${key} can be at most ${value}`,
    KeyNotFound: (key: string) => `The ${key} could not be found`,
    KeyMustContainT: (key: string, t: string) => `${key} must contain ${t}`,
    KeyMustBeIncludedInValues: (
      key: string,
